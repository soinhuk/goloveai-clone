...
export default function Create() {
  ...
  // Pause videos when not on style selection step
  useEffect(() => {
    const videos = document.querySelectorAll('video[data-style-video]')
  ...
  const canContinue = () => {
    switch (step) {
      ...
      case 2: return selectedPersonality.length > 0 && selectedVoice
  ...
  return (
    <div className="min-h-screen bg-[#0F0E0F] text-white pb-20">
      ...
      <div className="flex items-center gap-4 px-4 py-4 border-b border-white/[6%]">
        <button onClick={() => step > 0 ? setStep(step - 1) : null} className="flex items-center justify-center size-9 rounded-full hover:bg-white/[6%] transition-colors">
      ...
      <div className="max-w-2xl mx-auto px-4 py-6">
        ...
        {step === 0 && (
          <div className="space-y-6">
            ...
            {/* Style Cards with Video Background - h-[200px] rounded-[16px] */}
            <div className="grid grid-cols-2 gap-4">
              {(['realistic', 'anime'] as const).map(s => (
                ...
                >
                  ...
                  {/* Checkmark Badge */}
                  {style === s && (
                    <span className="absolute top-3 right-3 size-6 rounded-full bg-gradient-to-r from-[#d05bf8] to-[#ff18a0] flex items-center justify-center shadow-lg">
            ...
            <button
              ...
              className="w-full py-3 rounded-[16px] bg-gradient-to-r from-[#d05bf8]/10 to-[#ff18a0]/10 border border-[#d05bf8]/20 text-white flex items-center justify-center gap-2 hover:border-[#d05bf8]/40 transition-all"
            >
          ...
          <div className="space-y-6">
            <div>
              ...
              <input
                ...
                className="w-full bg-white/[4%] border border-white/[6%] rounded-xl py-3 px-4 text-white placeholder:text-white/30 focus:outline-none focus:border-[#d05bf8]/40"
              />
            </div>
            <div>
                ...
                {SKIN_TONES.map(c => (
                  <button
                    ...
                    className={`size-10 rounded-full border-2 transition-all ${
                      skinTone === c ? 'border-[#d05bf8] scale-110 shadow-lg shadow-[#d05bf8]/20' : 'border-transparent hover:scale-105'
                    }`}
                    style={{ background: c }}
                  />
                ))}
              ...
              <div className="flex gap-3">
                {HAIR_COLORS.map(c => (
                  <button
                    ...
                    className={`size-10 rounded-full border-2 transition-all ${
                      hairColor === c ? 'border-[#d05bf8] scale-110 shadow-lg shadow-[#d05bf8]/20' : 'border-transparent hover:scale-105'
                    }`}
                    style={{ background: c }}
                  />
                ))}
                ...
                {PERSONALITIES.map(p => (
                  <button
                    ...
                    className={`px-4 py-2 rounded-full text-[12px] font-medium transition-all ${
                      selectedPersonality.includes(p)
                        ? 'bg-gradient-to-r from-[#d05bf8] to-[#ff18a0] text-white'
                        : 'bg-white/[4%] text-white/50 border border-white/[6%] hover:border-white/[15%]'
                    }`}
                  >
                    {p}
                  </button>
              ...
              <div className="grid grid-cols-2 gap-3">
                {RELATIONSHIPS.map(r => (
                  <button
                    ...
                    className={`px-4 py-3 rounded-[16px] text-[13px] font-medium transition-all text-left ${
                      selectedRelationship === r
                        ? 'bg-gradient-to-r from-[#d05bf8]/20 to-[#ff18a0]/20 border border-[#d05bf8]/40 text-white'
                        : 'bg-white/[4%] text-white/50 border border-white/[6%] hover:border-white/[15%]'
                    }`}
                  >
                    {r}
        ...
        <div className="mt-12 space-y-8">
          ...
          <div className="space-y-2">
            <span className="text-[10px] font-bold text-[#d05bf8] uppercase tracking-wider">First Step</span>
            <h3 className="text-[16px] font-[600]">Design Her Look From Scratch</h3>
            <p className="text-[13px] text-white/50 leading-[20px]">
              Go realistic or anime. Set her skin tone, eyes, hair, body type, and style.
            </p>
          </div>

          {/* Second Step */}
          <div className="space-y-2">
            <span className="text-[10px] font-bold text-[#d05bf8] uppercase tracking-wider">Second Step</span>
            <h3 className="text-[16px] font-[600]">Shape Her Into Someone Real</h3>
            <p className="text-[13px] text-white/50 leading-[20px]">
              Create AI Girlfriend with 40+ personality types or build one from scratch. Pick from 19 voices and 135 career roles.
            </p>
          </div>

          {/* Third Step */}
          <div className="space-y-2">
            <span className="text-[10px] font-bold text-[#d05bf8] uppercase tracking-wider">Third Step</span>
            <h3 className="text-[16px] font-[600]">Define What You Two Are</h3>
            <p className="text-[13px] text-white/50 leading-[20px]">
              Choose from 29 relationship types — from a flirty stranger to a devoted partner.
            </p>
          </div>
        </div>
        ...
        <div className="mt-12 space-y-4">
          ...
          {[
            {
              q: 'Can I Create AI Girlfriend Free?',
              a: 'Yes — no card needed. Build her and start chatting right now.'
            },
            {
              q: 'How do I Create Your AI Girlfriend from scratch?',
              a: 'Pick her look, set her personality, choose your dynamic. Done in under 2 minutes.'
            },
            {
              q: 'Can I Create Girlfriend AI with a custom personality?',
              a: "Absolutely. Choose from 40+ types or design her personality yourself — she's 100% yours."
            },
            {
              q: 'What makes this AI GF different from other chatbots?',
              a: 'You Create Your AI Girlfriend your way — her face, voice, and personality are all set by you, not by default.'
            },
          ].map((faq, i) => (
      ...
      {/* Bottom Bar with Design AI + Continue */}
      <div className="fixed bottom-0 left-0 right-0 z-50 flex items-center gap-3 px-4 py-3 bg-[#0F0E0F]/95 backdrop-blur-md border-t border-white/[6%]">
        ...
        <button
          onClick={() => step < 3 && canContinue() && setStep(step + 1)}
          disabled={!canContinue()}
          className={`flex-1 py-[10px] rounded-full text-[14px] font-semibold transition-all ${
            canContinue()
              ? 'bg-gradient-to-r from-[#d05bf8] to-[#ff18a0] text-white hover:shadow-[0_0_20px_rgba(208,91,248,0.4)]'
        ...
        <div className="fixed inset-0 z-[200] bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-[#181718] rounded-[22px] border border-white/[10%] w-full max-w-md p-6 space-y-4">
            ...
            <div className="flex gap-2">
              <button className="px-3 py-1.5 rounded-full bg-white/[4%] border border-white/[6%] text-[12px] text-white/60 flex items-center gap-1">
                Style <span className="text-white/80 capitalize">{style}</span> <ChevronDown size={10} />
              </button>
              <button className="px-3 py-1.5 rounded-full bg-white/[4%] border border-white/[6%] text-[12px] text-white/60 flex items-center gap-1">
                Gender <span className="text-white/80 capitalize">{gender}</span> <ChevronDown size={10} />
              </button>
            </div>
            <textarea
              value={aiPrompt}
            ...
            <button
              ...
              className={`w-full py-3 rounded-xl text-[14px] font-semibold transition-all ${
                aiPrompt.trim()
                  ? 'bg-gradient-to-r from-[#d05bf8] to-[#ff18a0] text-white cursor-pointer'
  ...
  return (
    <div className="border border-white/[6%] rounded-[16px] overflow-hidden">
      <button
        ...
        className="w-full flex items-center justify-between px-4 py-4 text-left hover:bg-white/[2%] transition-all"
      >
        <span className="text-[14px] font-[500] pr-4">{question}</span>
        <ChevronDown
          size={16}
      ...
      {open && (
        <div className="px-4 pb-4 text-[13px] text-white/50 leading-[20px] animate-in fade-in duration-200">
          {answer}
        </div>
      )}
    </div>
